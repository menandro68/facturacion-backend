...
// PDF MEDIA CARTA HORIZONTAL - 8.5 x 5.5 (612 x 396) - LLENA TODA LA PAGINA
router.get('/:id/pdf', verifyToken, tenantGuard, async (req, res) => {
  try {
    ...
    const data = invoice.rows[0];
    const PDFDocument = require('pdfkit');
    // Tamano fisico real: 8.5 ancho x 5.5 alto = 612 x 396 puntos (horizontal)
    const doc = new PDFDocument({ margin: 25, size: [612, 396] });
    const esElectronica = ['E31', 'E32', 'E34'].includes(data.ncf_tipo);
    const tituloDocumento = {
      'E31': 'FACTURA CREDITO FISCAL ELECTRONICA',
    ...
    const rightW = col / 2;
    doc.fontSize(esElectronica ? 11 : 14).font('Helvetica-Bold')
       .text(tituloDocumento, rightX, 12, { width: rightW, align: 'right' });
    doc.fontSize(10).font('Helvetica')
       .text(`NCF: ${data.ncf || 'N/A'}`, rightX, 32, { width: rightW, align: 'right' });
    doc.fontSize(9).text(`Estado: ${data.estado.toUpperCase()}`, rightX, 45, { width: rightW, align: 'right' });
    doc.fontSize(9).text(`Fecha: ${data.fecha_emision ? new Date(data.fecha_emision).toLocaleDateString('es-DO') : new Date().toLocaleDateString('es-DO')}`, rightX, 56, { width: rightW, align: 'right' });
    ...
    doc.fillColor(azul).fontSize(11).font('Helvetica-Bold')
       .text('CLIENTE', M + 10, y + 8);
    doc.fillColor(negro).fontSize(11).font('Helvetica-Bold')
       .text(data.cliente_nombre || 'Consumidor Final', M + 10, y + 26, { width: blockW - 20 });
    doc.fontSize(9).font('Helvetica')
       .text(`RNC/Cedula: ${data.rnc_cedula || 'N/A'}`, M + 10, y + 44)
       .text(`Tel: ${data.cliente_telefono || 'N/A'}`, M + 10, y + 58)
       .text(`Dir: ${data.cliente_direccion || 'N/A'}`, M + 10, y + 72, { width: blockW - 20 });

    const cx = M + blockW + gap;
    doc.rect(cx, y, blockW, blockH).fill(gris).stroke('#E2E8F0');
    doc.fillColor(azul).fontSize(11).font('Helvetica-Bold')
       .text('CONDICIONES', cx + 10, y + 8);
    const condMap = { contado: 'Contado', '7_dias': '7 Dias', '15_dias': '15 Dias', '30_dias': '30 Dias', '45_dias': '45 Dias', '60_dias': '60 Dias' };
    doc.fillColor(negro).fontSize(11).font('Helvetica-Bold')
       .text(condMap[data.cliente_condiciones] || 'Contado', cx + 10, y + 26);
    doc.fontSize(9).font('Helvetica')
       .text(`Vendedor: ${data.vendedor_nombre || 'N/A'}`, cx + 10, y + 44)
       .text(`Negocio: ${data.cliente_negocio || 'N/A'}`, cx + 10, y + 58, { width: blockW - 20 });

    y += blockH + 10;

    // TABLA DE ITEMS - 6 columnas distribuidas en 562pt (col = 562)
    const colDescX = M + 8;
    const colDescW = 200;
    const colCantX = M + 212;
    const colCantW = 40;
    const colPUnitX = M + 256;
    const colPUnitW = 65;
    const colSubX = M + 325;
    const colSubW = 75;
    const colItbisX = M + 404;
    const colItbisW = 60;
    const colTotalX = M + 468;
    const colTotalW = col - (468 - M);

    doc.rect(M, y, col, 18).fill(azul);
    ...
    const tw = 220;
    const tx = M + col - tw;
    doc.rect(tx, y, tw, 16).fill(gris);
    doc.fillColor(negro).fontSize(9).font('Helvetica')
       .text('Subtotal:', tx + 10, y + 4)
       .text(`RD$ ${parseFloat(data.subtotal).toLocaleString('es-DO', {minimumFractionDigits: 2})}`, tx, y + 4, { width: tw - 10, align: 'right' });
    y += 16;
    doc.rect(tx, y, tw, 16).fill(gris);
    doc.fillColor(negro).fontSize(9).font('Helvetica')
       .text('ITBIS (18%):', tx + 10, y + 4)
       .text(`RD$ ${parseFloat(data.itbis).toLocaleString('es-DO', {minimumFractionDigits: 2})}`, tx, y + 4, { width: tw - 10, align: 'right' });
    y += 16;
    doc.rect(tx, y, tw, 22).fill(azul);
    doc.fillColor('white').fontSize(12).font('Helvetica-Bold')
       .text('TOTAL:', tx + 10, y + 6)
       .text(`RD$ ${parseFloat(data.total).toLocaleString('es-DO', {minimumFractionDigits: 2})}`, tx, y + 6, { width: tw - 10, align: 'right' });
    ...
    if (esElectronica) {
      ...
      const qrData = `https://ecf.dgii.gov.do/ecf/ConsultaTimbre?RncEmisor=${data.empresa_rnc || ''}&ENCF=${data.ncf || ''}&MontoTotal=${parseFloat(data.total).toFixed(2)}&FechaEmision=${data.fecha_emision ? new Date(data.fecha_emision).toISOString().split('T')[0] : ''}&CodigoSeguridad=${data.codigo_seguridad || ''}`;
      try {
        const qrPng = await QRCode.toBuffer(qrData, { width: 120, margin: 1 });
        doc.image(qrPng, M, qrY, { width: 60, height: 60 });
        doc.fillColor(azul).fontSize(8).font('Helvetica-Bold')
           .text('VALIDACION DGII (e-CF)', M + 66, qrY);
        doc.fillColor(negro).fontSize(7).font('Helvetica')
           .text(`Codigo: ${data.codigo_seguridad || '-'}`, M + 66, qrY + 12)
           .text(`Vence: ${data.fecha_vencimiento_encf ? new Date(data.fecha_vencimiento_encf).toLocaleDateString('es-DO') : '-'}`, M + 66, qrY + 24)
           .text('Escanee el QR para validar en DGII', M + 66, qrY + 40, { width: 200 });
      } catch (qrError) {
        // continuar sin QR
      }
    }
