...
router.get('/:id/pdf', verifyToken, tenantGuard, async (req, res) => {
  try {
    ...
    // ENCABEZADO - banda superior compacta (60pt)
    doc.rect(0, 0, W, 60).fill(azulOscuro);
    doc.fillColor('white').fontSize(16).font('Helvetica-Bold')
       .text(data.empresa_nombre || 'Mi Empresa', M, 12, { width: col / 2 });
    doc.fontSize(8).font('Helvetica')
       .text(`RNC: ${data.empresa_rnc || 'N/A'}`, M, 34, { width: col / 2 });
    doc.fontSize(8).text(data.empresa_email || '', M, 46, { width: col / 2 });
    ...
    doc.fontSize(9).font('Helvetica')
       .text(`NCF: ${data.ncf || 'N/A'}`, rightX, 28, { width: rightW, align: 'right' });
    doc.fontSize(8).text(`Estado: ${data.estado.toUpperCase()}    Fecha: ${data.fecha_emision ? new Date(data.fecha_emision).toLocaleDateString('es-DO') : new Date().toLocaleDateString('es-DO')}`, rightX, 42, { width: rightW, align: 'right' });
    if (data.numero_factura) {
      doc.fontSize(8).font('Helvetica-Bold')
         .text(`Factura No.: ${String(data.numero_factura).padStart(8, '0')}`, rightX, 53, { width: rightW, align: 'right' });
    }

    let y = 70;

    // BLOQUE CLIENTE/CONDICIONES (75pt alto)
    const blockH = 75;
    ...
    doc.fillColor(negro).fontSize(11).font('Helvetica-Bold')
       .text(data.cliente_nombre || 'Consumidor Final', M + 10, y + 24, { width: (col / 2) - 15 });
    doc.fontSize(9).font('Helvetica')
       .text(`RNC/Cédula: ${data.rnc_cedula || 'N/A'}`, M + 10, y + 41)
       .text(`Tel: ${data.cliente_telefono || 'N/A'}    Dir: ${data.cliente_direccion || 'N/A'}`, M + 10, y + 56, { width: (col / 2) - 15 });
    const rx = M + col / 2 + 10;
    ...
    doc.fillColor(negro).fontSize(11).font('Helvetica-Bold')
       .text(condMap[data.cliente_condiciones] || 'Contado', rx, y + 24);
    doc.fontSize(9).font('Helvetica')
       .text(`Vendedor: ${data.vendedor_nombre || 'N/A'}`, rx, y + 41)
       .text(`Negocio: ${data.cliente_negocio || 'N/A'}`, rx, y + 56, { width: col / 2 - 15 });
    ...
    const colDescX = M + 6;
    const colDescW = 220;
    const colCantX = M + 232;
    const colCantW = 40;
    const colPUnitX = M + 278;
    const colPUnitW = 70;
    const colSubX = M + 354;
    const colSubW = 75;
    const colItbisX = M + 435;
    const colItbisW = 60;
    const colTotalX = M + 502;
    const colTotalW = col - (502 - M);
    ...
    const tw = 220;
    const tx = M + col - tw;
    doc.rect(tx, y, tw, 18).fill(gris).stroke(grisBorde);
    doc.fillColor(negro).fontSize(10).font('Helvetica')
       .text('Subtotal:', tx + 10, y + 5)
       .font('Helvetica-Bold')
       .text(`RD$ ${parseFloat(data.subtotal).toLocaleString('es-DO', {minimumFractionDigits: 2})}`, tx, y + 5, { width: tw - 10, align: 'right' });
    y += 18;
    doc.rect(tx, y, tw, 18).fill(gris).stroke(grisBorde);
    doc.fillColor(negro).fontSize(10).font('Helvetica')
       .text('ITBIS (18%):', tx + 10, y + 5)
       .font('Helvetica-Bold')
       .text(`RD$ ${parseFloat(data.itbis).toLocaleString('es-DO', {minimumFractionDigits: 2})}`, tx, y + 5, { width: tw - 10, align: 'right' });
    y += 18;
    ...
    // E-CF (si aplica) - a la izquierda paralelo a totales
    if (esElectronica) {
      const qrYStart = y - 36;
      const qrData = `https://ecf.dgii.gov.do/ecf/ConsultaTimbre?RncEmisor=${data.empresa_rnc || ''}&ENCF=${data.ncf || ''}&MontoTotal=${parseFloat(data.total).toFixed(2)}&FechaEmision=${data.fecha_emision ? new Date(data.fecha_emision).toISOString().split('T')[0] : ''}&CodigoSeguridad=${data.codigo_seguridad || ''}`;
      try {
        const qrPng = await QRCode.toBuffer(qrData, { width: 90, margin: 1 });
        doc.image(qrPng, M, qrYStart, { width: 55, height: 55 });
        doc.fillColor(azul).fontSize(7).font('Helvetica-Bold')
           .text('VALIDACION DGII (e-CF)', M + 60, qrYStart);
        doc.fillColor(negro).fontSize(6).font('Helvetica')
           .text(`Codigo: ${data.codigo_seguridad || '-'}`, M + 60, qrYStart + 10)
           .text(`Vence: ${data.fecha_vencimiento_encf ? new Date(data.fecha_vencimiento_encf).toLocaleDateString('es-DO') : '-'}`, M + 60, qrYStart + 20)
           .text('Escanee el QR para validar en DGII', M + 60, qrYStart + 32, { width: 200 });
      } catch (qrError) {
        // continuar sin QR
      }
    }
    ...
    doc.fillColor(grisTexto).fontSize(7).font('Helvetica')
       .text('Este documento es valido como comprobante fiscal', M, footerY + 12, { width: col, align: 'center' });
